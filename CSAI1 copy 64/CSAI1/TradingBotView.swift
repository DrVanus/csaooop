...
struct TradingBotView: View {
    ...
    @State private var navigateToManage: Bool = false  // State variable for programmatic navigation
    
    // MARK: - For AI Chat – using the shared view model from your repository
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            // Hidden NavigationLink to push BotManagementView
            NavigationLink(
                destination: BotManagementView(), // Make sure BotManagementView is declared separately
                isActive: $navigateToManage
            ) {
                EmptyView()
            }
            
            // Main content switched by selected mode
            switch selectedMode {
            case .aiChat:
        ...
        // Let the parent NavigationView handle the nav bar. If you want a completely custom bar,
        // keep .navigationBarHidden(true) but ensure you have a valid nav context above this.
...
extension TradingBotView {
    private var customNavBar: some View {
        VStack(spacing: 0) {
            // Top row: Back button, title, and Manage button.
            HStack {
                Button {
                ...
                // Button that sets navigateToManage to true
                Button(action: {
                    // Tapping Manage pushes BotManagementView via the hidden NavigationLink
            ...
            // Segmented control for AI Chat / DCA / Grid / Signal
...
// Remove duplicate BotManagementView and BotManagementViewModel declarations from this file.
// Those are declared elsewhere in your project to avoid redeclaration errors.
