...
struct TradingBotView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            // Custom navigation area (excluding Manage button)
            customNavBar
            
            // Main content area switched by selected mode
            switch selectedMode {
            case .aiChat:
        ...
        .navigationBarTitle("Trading Bot", displayMode: .inline)
        // Place the Manage button in the standard toolbar:
        .toolbar {
            ToolbarItem(placement: .navigationBarTrailing) {
                NavigationLink(destination: BotManagementView()
                                .onAppear { print("BotManagementView appeared") }
                ) {
                    Text("Manage")
