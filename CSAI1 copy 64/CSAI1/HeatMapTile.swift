...
class HeatMapVM: ObservableObject {
    ...
    func fetchHeatMapData() {
        guard let url = URL(string:
            "https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=50&page=1&sparkline=true&price_change_percentage=\(timeframe.rawValue)"
        ) else { return }
                ...
                HStack {
                    ...
                    Picker("", selection: $selectedTimeframe) {
                        ForEach(HeatMapTimeframe.allCases) { tf in
                            Text(tf.rawValue.uppercased()).tag(tf)
                        }
                    }
...
struct HeatMapCard_Previews: PreviewProvider {
    static var previews: some View {
        HeatMapCard(timeframe: .oneDay, showWatchlistOnly: false)
